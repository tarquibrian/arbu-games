...
- **Misiones diarias (0009) + rachas (0010)** — la capa hábito del MVP, ambas con el mismo principio: **el progreso no se persiste**, se computa de las acciones que ya existen (`trees.created_at`, `tree_validations.created_at`). Un contador escribible es superficie de fraude y se desincroniza del dato real; lo único que se guarda es el *canje* de la misión (`mission_claims`, con `unique(user, mission, day)` como anti doble-pago). Las misiones del día se eligen determinísticamente por hash de (fecha, id) — sin cron ni tabla de asignaciones. La racha es el tramo de días consecutivos que termina **hoy o ayer** (si actuaste ayer sigue viva). Ambos cortan el día a medianoche **hora de Bolivia**. La insignia "Guardián Activo" usa `best` de `my_streak()`.
- **Onboarding**: 3 slides en español que cuentan el loop real (mapear → validar 1+3 → canjear). El carrusel necesita `getItemLayout` + `onScrollToIndexFailed`: sin eso `scrollToIndex` no resuelve el offset y la lista no pagina.
- **Máquina de estados del registro (0012)** — cierra el agujero que dejaba el pago-sólo-al-validar: un pendiente que juntaba 2 verificaciones no le pagaba a nadie nunca. `pending → estancado → validated`, o `→ unverifiable` si varias personas distintas reportan "fui y no está" (`report_tree_not_found()` activa `tree_reports`). **"Estancado" NO se persiste**: es función pura de (created_at, validations_count, now), expuesta como columna computada `is_stalled` de PostgREST y por `verification_queue()`, que prioriza los estancados y devuelve la recompensa con el bono ya aplicado. El bono va a los **verificadores** (el doc pide "atraer verificadores"); el registrante cobra la base. `unverifiable` sí se persiste: es conclusión con evidencia y es terminal.
- **Actividad del comercio (0013)** — Hipótesis 3. `merchant_activity()`, `merchant_coupon_stats()` y `merchant_recent_redemptions()`, todas **security definer acotadas por `merchant_members`**. Son RPCs y no una policy de RLS a propósito: `coupon_redemptions` guarda el `user_id` del ciudadano, así que abrir esas filas le daría al comercio acceso a QUIÉN canjeó. El comercio ve actividad, nunca identidad — hay un test sobre la *firma* de las funciones que falla si alguien agrega `user_id` al `returns table`.
- **Admin `/arboles`** — tabla simple de árboles + métricas del piloto (vista `pilot_metrics`), que es lo que el doc deja dentro del MVP para poder evaluar las hipótesis. "Estancado" se muestra como estado propio aunque en la base siga siendo `pending`, así que ese filtro se aplica en memoria y no en el `where`.
- **Schema**: `apps/mobile/supabase/migrations/0001`–`0013` — `trees`, `tree_validations`, `tree_reports`, `species`, `coupons`, `coupon_redemptions`, `merchants`, `merchant_members`, `admin_users`, `wallet_transactions`, `missions`/`mission_claims`, `app_config` (server-tunable knobs like `earn_rate`), all with RLS + server-authoritative triggers/RPCs (client never sets `status`, `validations_count`, `coins`, etc. directly — los grants de `trees`/`tree_validations` son **por columna** justamente para eso).
- **⚠️ `redeem_coupon` está definida dos veces**: 0001 la marca `used` de entrada para cupones `on_site` (haría irredimible todo cupón de mostrador) y **0004 la redefine a `claimed` siempre**. Leer sólo 0001 lleva a reportar un bug que no existe; lo mismo con `validate_redemption` / `is_merchant_member`.
- **Datos que producción necesita van en migraciones, no en `seed.sql`**: `supabase db push` (el camino de deploy) no corre el seed. Por eso el catálogo de misiones está en 0009 y el de **especies en 0011** (antes vivía en el seed: producción habría arrancado con cero especies). `seed.sql` queda sólo con comercios/cupones de prueba, que en producción se crean desde el admin. Runbook completo de deploy en `docs/deploy.md`.
...
| `stalled_lifecycle` | Estancado computado, cola priorizada, bono de rescate, y el estado terminal por evidencia |
| `merchant_activity` | Aislamiento entre comercios, y que la firma de los RPC no exponga identidad del ciudadano |
